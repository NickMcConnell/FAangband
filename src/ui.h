...
struct region {
  ...
  int page_rows;   /* non-positive value is relative to the bottom 
		    * of the screen */
};
...
struct event_listener
{
  ...
  event_set events;    /* Set of events to which this listener 
			* has subscribed */
};
...
event_type run_event_loop(event_target *parent, bool forever, 
 ...
 * dispatching.  A Panel has ownership of a Region, is a Container for Event 
 * Listeners,
 ...
 */
typedef void (*display_list_f)(menu_type *menu, int cursor, int *top, 
			       region *);
...
struct menu_item
{
  ...
  char sel;		/* Character used for selection, 
				 * if special-purpose bindings 
				 * are desired. */
...
typedef enum {
  ...
  MN_REL_TAGS   = 0x0100, /* Tags are associated with the view, 
			   * not the element */
  ...
  MN_CASELESS_TAGS = 0x0800, /* Tag selections can be made regardless of the 
			      *case of the key pressed. - i.e. 'a' activates 
			      * the line tagges 'A'. */

  MN_DBL_TAP    = 0x1000, /* double tap for selection; 
			   * single tap is cursor movement */
  MN_NO_ACT     = 0x2000, /* Do not invoke the specified action; 
			   * menu selection only */
  MN_PAGE       = 0x4000, /* Use full-page scrolling rather than small 
			   * increment */
  ...
  /* key-binding. (Useful for lower case alternate) */
} menu_flags;
...
struct menu_iter {
  ...
  /* To support "hidden" items, it uses 3-level logic: 
   * 0 = no  1 = yes 2 = hide */
 ...
 * 
 * Arguments:
 *  object_list - optional ordered subset of menu OID.  
 * If NULL, cursor is used for OID
