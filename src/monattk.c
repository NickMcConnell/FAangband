...
static int monster_critical(int dice, int sides, int dam)
{
  ...
  if ((dam < 20) && (dam < randint1(100))) return (0);
  ...
  for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
      ...
      if (!effect || check_hit(power, rlev, terrain_bonus, m_idx))
	{
	  ...
	  if (do_stun)
		...
		case 0: k = 0; break;
		case 1: k = randint1(5); break;
		case 2: k = randint1(8) + 8; break;
		case 3: k = randint1(15) + 15; break;
		case 4: k = randint1(25) + 25; break;
		case 5: k = randint1(35) + 35; break;
		case 6: k = 60; break;
		default: k = 100; break;
		}
  ...
  if (range < MAX_SIGHT) 
    {
      ...
      (void)project(m_idx, range, py, px, dam, typ, flg, 0,(byte)(range * 10));
    }
...
bool make_attack_ranged(monster_type *m_ptr, int attack)
{
  ...
  switch (attack)
	...
	if (r_ptr->blow[1].d_dice) 
	  damage += damroll(r_ptr->blow[1].d_dice, r_ptr->blow[1].d_side) / 2;
	if (r_ptr->blow[2].d_dice) 
	  damage += damroll(r_ptr->blow[2].d_dice, r_ptr->blow[2].d_side) / 2;
	if (r_ptr->blow[3].d_dice) 
	  damage += damroll(r_ptr->blow[3].d_dice, r_ptr->blow[3].d_side) / 2;
	...
	/* For weak Udun-magic casters, a column of hellfire. */
	else if (rf_has(r_ptr->flags, RF_UDUN_MAGIC))
	  {
	    if (blind) msg_format("%^s murmurs darkly.", m_name);
	      ...
	      msg_format("%^s gestures, and you are enveloped in hellfire.", 
			 m_name);
	...
	/* Column of fire for everyone else. */
	else
	  {
	    ...
	    else msg_format("%^s gestures, and you are enveloped in fire.", 
			    m_name);
	...
	/* Have we seen them at any point?  
	 * If so, we will learn about the spell. */
	...
	if (spower < 7)
	...
	else if (spower < 55)
	  {
	    ...
	    else msg_format("%^s points at you, screaming words of peril!", 
			    m_name);
	    k = 4;
	  }
	else
	  {
	    if (blind) msg_format("%^s screams the word 'DIE!'", m_name);
	      ...
	      msg_format("%^s points at you, screaming the word DIE!", m_name);
	    k = 5;
	  }
