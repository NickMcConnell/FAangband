...
bool change_panel(int dy, int dx)
{
    ...
    if ((!p_ptr->depth) && small_town)
	x_max = DUNGEON_WID / 2;

    /* Bounds */
    if (y > y_max - SCREEN_HGT)
	y = y_max - SCREEN_HGT;
    if (y < y_min)
	y = y_min;
    if (x > x_max - SCREEN_WID)
	x = x_max - SCREEN_WID;
    if (x < x_min)
	x = x_min;
...
void verify_panel(void)
{
	...
	if (!p_ptr->depth) {
	    ...
	    x_max = ((p_ptr->stage < 151)
		     && !OPT(adult_dungeon)) ? (DUNGEON_WID / 2) -
		SCREEN_WID : (2 * DUNGEON_WID / 3) - SCREEN_WID;

	    /* Bounds */
	    if ((prow_min > y_max) && (py > y_max))
		prow_min = y_max;
	    if (prow_min < y_min)
		prow_min = y_min;
	    if ((pcol_min > x_max) && (px > x_max))
		pcol_min = x_max;
	    if (pcol_min < x_min)
		pcol_min = x_min;
	}
...
bool target_able(int m_idx)
{
    ...
    if (!projectable(py, px, m_ptr->fy, m_ptr->fx, 0))
	return (FALSE);
...
bool target_okay(void)
{
    ...
    if ((p_ptr->target_who == 0) && (p_ptr->target_what == 0))
	return (TRUE);
...
static void target_set_interactive_prepare(int mode)
{
	...
	for (x = panel_col_min; x <= panel_col_max; x++) {
	    ...
	    if (!OPT(expand_look) && !player_has_los_bold(y, x))
		continue;
						...
						cptr info)
{
	...
	if (cave_m_idx[y][x] > 0) {
		...
		while (1) {
			...
			sprintf(out_val, "%s%s%s%s (%s%s) [r,%s]", s1, s2, s3,
				m_name, m_desc, look_mon_host(cave_m_idx[y][x]),
				info);
			prt(out_val, 0, 0);
	...
	if (scan_floor(floor_list, &floor_num, y, x, 0x02)) {
		...
		if (floor_num == 1) {
		    ...
		    /* Message */
		    if (OPT(rogue_like_commands))
			sprintf(out_val, "%s%s%sa pile of %d items [x,%s]", s1,
				s2, s3, floor_num, info);
		    else
			sprintf(out_val, "%s%s%sa pile of %d items [l,%s]", s1,
				s2, s3, floor_num, info);
		}
	...
	if (boring || (feat > FEAT_INVIS)) {
	    ...
	    /* Display a message */
	    sprintf(out_val, "%s%s%s%s%s%s [%s]", s1, s2, s3, name, s4, s5,
		    info);
	    prt(out_val, 0, 0);
	    move_cursor_relative(y, x);
	    query = inkey_ex();
...
bool target_set_interactive(int mode)
{
	...
	if (flag && temp_n) {
	    ...
	    if (((mode & TARGET_KILL) && (cave_m_idx[y][x] > 0)
		 && target_able(cave_m_idx[y][x])) || ((mode & TARGET_OBJ)
						       && (cave_o_idx[y][x] > 0)
						       &&
						       target_able_obj
						       (cave_o_idx[y][x]))) {
	    ...
	    case '\xff':
		{
			...
			/* target_set_location(y, x); done = TRUE; */
			flag = FALSE;
			break;
		    } else;
		    /* Fall through */
	...
	/* Arbitrary grids */
		...
		if (x <= 0)
		    x = 0 + 1;
		else if (x >= DUNGEON_WID - 1)
		    x = DUNGEON_WID - 2;

		/* Slide into legality */
		if (y <= 0)
		    y = 0 + 1;
		else if (y >= DUNGEON_HGT - 1)
		    y = DUNGEON_HGT - 2;
	    }
	}
    }
			     ...
			     bool require_visible)
{
    ...
    *tx = 0;
	...
	if (use_view) {
	    if (!(cave_info[m_ptr->fy][m_ptr->fx] & (CAVE_VIEW)))
		continue;
	...
	/* Monster must be in los from the starting position (slower way) */
	    ...
	    if (!los(y0, x0, m_ptr->fy, m_ptr->fx))
		continue;
	}
...
bool get_aim_dir(int *dp)
{
    ...
    if (OPT(use_old_target) && target_okay())
	dir = 5;
	...
	case '.':
		...
		while (TRUE) {
		    ...
		    get_closest_los_monster(n, p_ptr->py, p_ptr->px, &y, &x,
					    TRUE);
