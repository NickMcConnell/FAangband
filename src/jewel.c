			...
			object_type *o_ptr)
{
  ...
  switch(purchase)
    ...
    case VUL_ACID:
      {
	if (o_ptr->percent_res[P_RES_ACID] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_ACID] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_ELEC:
      {
	if (o_ptr->percent_res[P_RES_ELEC] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_ELEC] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_FIRE:
      {
	if (o_ptr->percent_res[P_RES_FIRE] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_FIRE] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_COLD:
      {
	if (o_ptr->percent_res[P_RES_COLD] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_COLD] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_POIS:
      {
	if (o_ptr->percent_res[P_RES_POIS] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 100 * value)) 
	  {
	    o_ptr->percent_res[P_RES_POIS] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    ...
    case VUL_DARK:
      {
	if (o_ptr->percent_res[P_RES_DARK] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 60 * value)) 
	  {
	    o_ptr->percent_res[P_RES_DARK] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_CONFU:
      {
	if (o_ptr->percent_res[P_RES_CONFU] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 75 * value)) 
	  {
	    o_ptr->percent_res[P_RES_CONFU] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_SOUND:
      {
	if (o_ptr->percent_res[P_RES_SOUND] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_SOUND] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_SHARD:
      {
	if (o_ptr->percent_res[P_RES_SHARD] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_SHARD] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_NEXUS:
      {
	if (o_ptr->percent_res[P_RES_NEXUS] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 50 * value)) 
	  {
	    o_ptr->percent_res[P_RES_NEXUS] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_NETHR:
      {
	if (o_ptr->percent_res[P_RES_NETHR] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 80 * value)) 
	  {
	    o_ptr->percent_res[P_RES_NETHR] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_CHAOS:
      {
	if (o_ptr->percent_res[P_RES_CHAOS] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 80 * value)) 
	  {
	    o_ptr->percent_res[P_RES_CHAOS] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
    case VUL_DISEN:
      {
	if (o_ptr->percent_res[P_RES_DISEN] < RES_LEVEL_BASE) return(FALSE);
	if (!value) value = randint(6);
	if (take_money(on_credit, 0 - 100 * value)) 
	  {
	    o_ptr->percent_res[P_RES_DISEN] = RES_LEVEL_BASE + 5 * value;
	    return(TRUE);
	  }
	break;
      }
...
static int select_property(int temp_potential, int *property_list, 
			   const int choices, int *max_value, int rank_method,
			   object_type *o_ptr)
{
  int i, j, optimal = 0;
  int selection = 0;
  ...
  /* Initialise best */
  if (rank_method == CHEAPEST_FIRST) optimal = TOO_MUCH;
  ...
  for (i = 0; i < choices; i++)
      ...
      for (j = 0; j <= *max_value; j++)
	{
	  switch (rank_method)
	    ...
	    case DEAREST_FIRST:
	      {
		if ((prices[i][j] < TOO_MUCH) && (prices[i][j] > optimal)) 
		  {
		    optimal = prices[i][j];
		    selection = i;
		    current_value = j;
  ...
  *max_value = current_value;
  ...
  switch (rank_method)
    {
    case CHEAPEST_FIRST: if (optimal < TOO_MUCH) found_it = TRUE; break;
    case DEAREST_FIRST:  if (optimal > 0) found_it = TRUE; break;
    case FIND_MAX_VALUE: if (current_value > 0) found_it = TRUE; break;
    }
  
  
  /* Return if found */
  if (found_it) return (property_list[selection]);
  else return (0);
}
  ...
  while (!done)
      ...
      if (o_ptr->tval == TV_AMULET)
	{
	  switch(e_idx)
		...
		while (randint(10) != 10)
		  {
		    int curse = randint(CURSE_MAX);
		    if (curse > CURSE_BASE) get_quality(TRUE, curse, 0, o_ptr);
		    else break;
		  }
		...
		if (randint(2) == 1) 
		  {
		    get_quality(TRUE, RES_ELEC, rand_int(4) + 3, o_ptr);
		    if (randint(3) == 1) 
		      get_quality(TRUE, RES_ACID, rand_int(4) + 3, o_ptr);
		  }
		else 
		  {
		    get_quality(TRUE, RES_ACID, rand_int(4) + 3, o_ptr);
		...
		if (potential > 1000)
		  {
		    if (randint(10) == 1) 
		      get_quality(TRUE, RES_COLD, rand_int(4) + 5, o_ptr);
		    if (randint(10) == 1) 
		      get_quality(TRUE, RES_FIRE, rand_int(4) + 5, o_ptr);
		  }
		...
		if (potential < 1000) break;

		/* Bonus to magical item mastery */
		get_quality(TRUE, MAGIC_MASTERY, randint(bonus), o_ptr);
		...
		if (randint(5) == 1)
		  get_quality(TRUE, RES_ELEC, rand_int(4) + 3, o_ptr);
		if (randint(5) == 1)
		  get_quality(TRUE, RES_FIRE, rand_int(4) + 3, o_ptr);
		if (randint(5) == 1)
		  get_quality(TRUE, RES_ACID, rand_int(4) + 3, o_ptr);
		...
		if (randint(3500) > initial_potential)
		...
		if (randint(8) == 1) get_quality(FALSE, VUL_NETHR, 0, o_ptr);
		else if (randint(8) == 1) get_quality(FALSE, VUL_DISEN, 0, o_ptr);
		done = TRUE;
		break;
	      }
	    case EGO_AMULET_SHADOWS:
	      ...
	      {
		/* This is an exclusive club */
		if (potential < 1500) break;

		/* Creature of the night  */
		get_quality(TRUE, DARKNESS, 0, o_ptr);
		get_quality(TRUE, STEALTH, rand_int(bonus), o_ptr);
		get_quality(TRUE, RES_DARK, 6 - rand_int(bonus), o_ptr);
		get_quality(TRUE, VUL_LITE, 5 - rand_int(bonus), o_ptr);

		/* Maybe some infravision and perception */
		if (randint(4) == 1) 
		  get_quality(TRUE, INFRA,  1 + rand_int(3), o_ptr);
		if (randint(4) == 1) 
		  get_quality(TRUE, SEARCH,  1 + rand_int(3), o_ptr);
	    ...
	    case EGO_AMULET_SUSTENANCE:
	      /* min potential  50 */
	      {
		int sustains[8] = {SUST_STR, SUST_INT, SUST_WIS, SUST_DEX,
		...
		while (property > 0)
		  {
		    property = select_property(potential, sustains, 8, &max, 
					       RANDOM_CHOICE, o_ptr); 
		...
		get_quality(TRUE, ADD_DEX, 2 + rand_int(bonus), o_ptr);
		get_quality(TRUE, SUST_DEX, 0, o_ptr);
		get_quality(TRUE, STEALTH, 2 + rand_int(bonus), o_ptr);

		/* Better amulets get more */
		if (randint(500) < potential)
		  get_quality(TRUE, SEARCH, 2 + rand_int(bonus), o_ptr);
		if (randint(1000) < potential)
		  get_quality(TRUE, RES_NEXUS, 0, o_ptr);
		if (randint(1500) < potential)
		  get_quality(TRUE, SPEED, randint(bonus), o_ptr);
		...
		if (randint(8) == 1) get_quality(FALSE, VUL_CONFU, 0, o_ptr);
		else if (randint(8) == 1) 
      ...
      else if (o_ptr->tval == TV_RING)
	{
	  switch(e_idx)
		...
		if (randint(3) == 1) 
		  {
		    property = select_property(potential, sustains, 3, 
		...
		int property;
		int max_value = 0;
		int mobility[5] = {SEE_INVIS, FEARLESS, SEEING, RES_CONFU, 
				  RES_NEXUS};

		/* This is an exclusive club */
		if (potential < 500) break;
		...
		if (randint(2) == 1) get_quality(TRUE, RES_POIS, 0, o_ptr);
		...
		while (randint(potential) > 1000)
		  {
		    ...
		    if (o_ptr->percent_res[P_RES_POIS + k] != RES_LEVEL_BASE)
		...
		int tries = randint(bonus + 2);

		/* Pick a few properties */
		if (tries > 6) tries = 6;
		for (j = 0; j < tries; j++)
		  {
		    property = select_property(potential, utility, 5, 
					       &max_value, RANDOM_CHOICE, 
					       o_ptr);
		    get_quality(TRUE, property, max_value, o_ptr);
		  }
		...
		if (randint(2) == 1) 
		  {
		    get_quality(TRUE, RES_FIRE, rand_int(4) + 3, o_ptr);
		    if (randint(3) == 1) 
		      get_quality(TRUE, RES_COLD, rand_int(4) + 3, o_ptr);
		  }
		else 
		  {
		    get_quality(TRUE, RES_COLD, rand_int(4) + 3, o_ptr);
		...
		if (potential > 1000)
		  {
		    if (randint(10) == 1) 
		      get_quality(TRUE, RES_ACID, rand_int(4) + 5, o_ptr);
		    if (randint(10) == 1) 
		      get_quality(TRUE, RES_ELEC, rand_int(4) + 5, o_ptr);
		  }
		...
		int max_value = 5;
		int property;
		int stats[6] = {ADD_STR, ADD_INT, ADD_WIS, ADD_DEX, ADD_CON, 
				ADD_CHR};
		...
		for (j = 0; j < tries; j++)
		  {
		    property = select_property(potential, stats, 6, &max_value,
					       RANDOM_CHOICE, o_ptr);
		    get_quality(TRUE, property, -randint(5), o_ptr);
		  }

		/* Potential gained counts as lost */
		if (potential - initial_potential > initial_potential)
		  potential = 0;
		else potential = (2 * initial_potential) - potential;
		...
		if (randint(3) == 1) get_quality(FALSE, SEE_INVIS, 0, o_ptr);
		...
		int curse;
		int max_value = 0;
		int fickle[7] = {TELEPORT, NO_TELEPORT, CUT_RAND, HALLU_RAND,
				AGGRO_RAND, ATTRACT_DEMON, PARALYZE};
		while (1)
		  {
		    curse = select_property(potential, fickle, 7, &max_value,
					    RANDOM_CHOICE, o_ptr);
		...
		o_ptr->to_d += temp;
		o_ptr->to_h += temp;
		o_ptr->to_a += 3 + randint(5) + potential / 1000;
		
		/* Power over an element */
		get_quality(FALSE, IM_ACID + element, 0, o_ptr);
		  		
		/* Sometimes vulnerable to high elements... */
		if (randint(8) == 1) get_quality(FALSE, VUL_NEXUS, 0, o_ptr);
		if (randint(8) == 1) get_quality(FALSE, VUL_NETHR, 0, o_ptr);
		if (randint(8) == 1) get_quality(FALSE, VUL_CHAOS, 0, o_ptr);
		if (randint(8) == 1) get_quality(FALSE, VUL_DISEN, 0, o_ptr);
		
		/* ...but likely to sustain stats */
		if (randint(3) == 1) get_quality(FALSE, SUST_STR, 0, o_ptr);
		if (randint(3) == 1) get_quality(FALSE, SUST_INT, 0, o_ptr);
		if (randint(3) == 1) get_quality(FALSE, SUST_WIS, 0, o_ptr);
		if (randint(3) == 1) get_quality(FALSE, SUST_DEX, 0, o_ptr);
		if (randint(3) == 1) get_quality(FALSE, SUST_CON, 0, o_ptr);
		if (randint(3) == 1) get_quality(FALSE, SUST_CHR, 0, o_ptr);

		/* And with a powerful activation at a bargain price... */
		temp = rand_int(ACT_POWER_MAX - ACT_POWER_BASE + 1);
...
static void add_properties(object_type *o_ptr)
{
  ...
  if (randint(8) == 1)
    {
      ...
      for (i = 0; i < rounds; i++)
	{
	  choice = rand_int(VUL_MAX - VUL_BASE + 1);
  ...
  if ((randint(5000) < potential) && (potential >= 1000))
    {
      /* Determine number of loops. */
      rounds = randint(potential / 1000);
      
      for (i = 0; i < rounds; i++)
	{
	  choice = rand_int(CURSE_MAX - CURSE_BASE + 1);
  ...
  if ((potential > 1500) && (randint(3) != 1))
	...
	{
	  if ((randint(2) == 1) && (o_ptr->bonus_other[P_BONUS_SPEED] == 0))
	    {
	      get_quality(TRUE, SPEED, randint(10), o_ptr);
	    }
	  else
	    {
  ...
  if ((o_ptr->to_h > 0) && (randint(3) == 1)) 
    get_quality(TRUE, ADD_SKILL, randint(o_ptr->to_h), o_ptr);
  if ((o_ptr->to_d > 0) && (randint(3) == 1)) 
    get_quality(TRUE, ADD_DEADLINESS, randint(o_ptr->to_d), o_ptr);
  if ((o_ptr->to_a > 0) && (randint(3) == 1)) 
    get_quality(TRUE, ADD_AC, randint(o_ptr->to_a), o_ptr);

  /* Objects without an activation may get one now, if affordable */
  if ((randint(3) == 1) && !o_ptr->activation)
    get_activation(TRUE, rand_int(ACT_RANDOM_MAX - ACT_RANDOM_BASE + 1) + 
   ...
   */
  if (((randint(5) < 3) || (potential >= 2000)) && ((!pval) || (pval < 7)))
    {
      pval = randint(3);
      
      choice = randint(9);
      
      if (choice == 1)  get_quality(TRUE, ADD_STR, pval, o_ptr);
      if (choice == 2)  get_quality(TRUE, ADD_WIS, pval, o_ptr);
      if (choice == 3)  get_quality(TRUE, ADD_INT, pval, o_ptr);
      if (choice == 4)  get_quality(TRUE, ADD_DEX, pval, o_ptr);
      if (choice == 5)  get_quality(TRUE, ADD_CON, pval, o_ptr);
      if (choice == 6)  get_quality(TRUE, ADD_CHR, pval, o_ptr);
      if (choice == 7)  get_quality(TRUE, STEALTH, pval, o_ptr);
      if (choice == 8)  get_quality(TRUE, SEARCH, pval, o_ptr);
      if (choice == 9)  get_quality(TRUE, SPEED, pval, o_ptr);
  ...
  while ((potential >= 300) && (limit-- > 0))
	...
	{
	  if (o_ptr->to_a > 0) o_ptr->to_a += potential / 100;
	  potential = 0;
	}
    }
  ...
  {
    if ((o_ptr->to_a % 5 == 4) && (randint(2) == 1)) o_ptr->to_a++;
    else if ((o_ptr->to_a % 5 == 1) && (randint(2) == 1)) o_ptr->to_a--;
    else if ((o_ptr->to_a % 2 == 1) && (randint(4) != 1)) o_ptr->to_a++;
    
    if ((o_ptr->to_h % 5 == 4) && (randint(2) == 1)) o_ptr->to_h++;
    else if ((o_ptr->to_h % 5 == 1) && (randint(2) == 1)) o_ptr->to_h--;
    else if ((o_ptr->to_h % 2 == 1) && (randint(4) != 1)) o_ptr->to_h++;
    
    if ((o_ptr->to_d % 5 == 4) && (randint(2) == 1)) o_ptr->to_d++;
    else if ((o_ptr->to_d % 5 == 1) && (randint(2) == 1)) o_ptr->to_d--;
    else if ((o_ptr->to_d % 2 == 1) && (randint(4) != 1)) o_ptr->to_d++;
  }
}
...
static void j_make_terrible(object_type *o_ptr)
{
  ...
  if ((o_ptr->activation) && (randint(3) != 1)) o_ptr->activation = 0;
  ...
  for (i = 0; i < gauntlet_runs; i++)
    {
      ...
      if ((o_ptr->to_h > 0) && (randint(2) == 1)) wheel_of_doom = 2;
      else if ((o_ptr->to_a > 0) && (randint(2) == 1))  wheel_of_doom = 1;
	  ...
	  if (randint(2) == 1)
	    {
	      ...
	      o_ptr->to_h = -(5 + randint(7)) * 2;
	      if (randint(3) != 1) o_ptr->to_d = -(5 + randint(7)) * 2;
	    }
      ...
      if (wheel_of_doom == 3)
	{
	  if (randint(3) != 1)
		...
		{
		  for (j = 0; j < A_MAX; j++)
		    if (randint(4) == 1) o_ptr->bonus_stat[j] = penalty;
		}
      ...
      if (wheel_of_doom == 4)
	{
	  if (randint(4) == 1) o_ptr->bonus_other[P_BONUS_SHOTS] = 0;
	  if (randint(4) == 1) o_ptr->bonus_other[P_BONUS_MIGHT] = 0;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_ANIMAL] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_EVIL] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_UNDEAD] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_DEMON] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_ORC] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_TROLL] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_GIANT] = 10;
	  if (randint(3) == 1) o_ptr->multiple_slay[P_SLAY_DRAGON] = 10;
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_PERFECT_BALANCE);
	  if (randint(3) == 1) o_ptr->multiple_brand[P_BRAND_POIS] = 10;
	  if (randint(3) == 1) o_ptr->multiple_brand[P_BRAND_ACID] = 10;
	  if (randint(3) == 1) o_ptr->multiple_brand[P_BRAND_ELEC] = 10;
	  if (randint(3) == 1) o_ptr->multiple_brand[P_BRAND_FIRE] = 10;
	  if (randint(3) == 1) o_ptr->multiple_brand[P_BRAND_COLD] = 10;
	  ...
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_SLOW_DIGEST);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_FEATHER);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_LITE);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_REGEN);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_TELEPATHY);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_SEE_INVIS);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_FREE_ACT);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_HOLD_LIFE);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_FEARLESS);
	  if (randint(3) == 1) o_ptr->flags_obj &= ~(OF_SEEING);
	}
    }
...
extern bool design_ring_or_amulet(object_type *o_ptr, int lev)
{
  ...
  /* Assign a type. */
  if (!choose_type(o_ptr)) return FALSE;
