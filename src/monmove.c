  ...
  /*** Check passability of various features. ***/
	...
	if (feat == FEAT_LAVA) {
		...
		int hp = (rf_has(r_ptr->flags, RF_FORCE_MAXHP)
			  ? (r_ptr->hdice * r_ptr->hside) : (r_ptr->hdice *
							     (r_ptr->hside +
							      1) / 2));
...
static void make_confused_move(monster_type * m_ptr, int y, int x)
{
	...
	/* Water */
	    ...
	    if (mon_take_hit
		(cave_m_idx[m_ptr->fy][m_ptr->fx], 5 + m_ptr->maxhp / 20, &fear,
		 note_dies)) {
		death = TRUE;
	    } else {
		if (seen)
...
static void apply_monster_trap(monster_type * m_ptr, int y, int x, bool * death)
{
    ...
    /* Sanity check */
    if (!((feat >= FEAT_MTRAP_HEAD) && (feat <= FEAT_MTRAP_TAIL)))
	return;
	...
	if (!(rf_has(r_ptr->flags, RF_FLYING))
	    ...
	    if (m_ptr->ml)
		msg_format("%^s avoids your netted trap.", m_name);
	    trap_hit = FALSE;
	}
    }
	...
	if (!(rf_has(r_ptr->flags, RF_PASS_WALL))) {
	    if (m_ptr->ml)
		msg_format("%^s ignores your spirit trap.", m_name);
	    trap_hit = FALSE;
	}
    }
	...
	/* Confusion trap */
		...
		if (m_ptr->confused) {
		    ...
		    if (m_ptr->ml)
			msg_format("%^s is more confused.", m_name);
		} else {
...
static void process_move(monster_type * m_ptr, int ty, int tx, bool bash)
{
	...
	if (cave_feat[ny][nx] == FEAT_RUNE_MANA) {
	    ...
	    if (m_ptr->ml)
		msg_format("%^s loses some mana.", m_name);
	}
