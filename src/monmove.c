 ...
 */
static void remove_useless_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p, 
				  u32b *f7p, bool los)
{
  monster_type *m_ptr = &m_list[m_idx];
  monster_race *r_ptr = &r_info[m_ptr->r_idx];
...
static void apply_monster_trap(monster_type *m_ptr, int y, int x, bool *death)
{
  ...
  if (feat == FEAT_MTRAP_NET)
    {
      if (!(r_ptr->flags2 & (RF2_FLYING)) && (rand_int(3) != 0 || 
					      (RF2_PASS_WALL)))
	{
  ...
  if ((trap_hit) && (r_ptr->flags2 & (RF2_SMART)) && 
      (randint(dis_chance) > p_ptr->skill_dis - 15))
    {
  ...
  if (trap_hit)
      ...
      else if (feat == FEAT_MTRAP_STURDY)
	{
      ...
      else if (randint(p_ptr->skill_dis) > 50 + r_ptr->level) 
	...
	msg_format("%^s flees in terror!", m_name);
      ...
      if ((!(r_ptr->flags2 & (RF2_STUPID))) &&
	  ((rand_int(4) == 0) || (m_ptr->hp < m_ptr->maxhp / 2))) 
...
static void process_move(monster_type *m_ptr, int ty, int tx, bool bash)
{
  ...
  if (do_move)
      ...
      if ((m_ptr->ty == ny) && (m_ptr->tx == nx))
	{
      ...
      if (r_ptr->flags2 & RF2_SMASH_WALL)
	{
	  int xx, yy;
	  feature_type *f_ptr;
	   ...
	   this_o_idx = next_o_idx)
	{
	  ...
	  if ((r_ptr->flags2 & (RF2_TAKE_ITEM)) ||
	      (r_ptr->flags2 & (RF2_KILL_ITEM)))
	    {
	      ...
	      if (o_slay(o_ptr, P_SLAY_DRAGON)) flg3 |= (RF3_DRAGON);
	      if (o_slay(o_ptr, P_SLAY_TROLL)) flg3 |= (RF3_TROLL);
	      if (o_slay(o_ptr, P_SLAY_GIANT)) flg3 |= (RF3_GIANT);
	      if (o_slay(o_ptr, P_SLAY_ORC)) flg3 |= (RF3_ORC);
	      if (o_slay(o_ptr, P_SLAY_DEMON)) flg3 |= (RF3_DEMON);
	      if (o_slay(o_ptr, P_SLAY_UNDEAD)) flg3 |= (RF3_UNDEAD);
	      if (o_slay(o_ptr, P_SLAY_ANIMAL)) flg3 |= (RF3_ANIMAL);
	      if (o_slay(o_ptr, P_SLAY_EVIL)) flg3 |= (RF3_EVIL);
...
static void process_monster(monster_type *m_ptr)
{
  ...
  if ((r_ptr->flags6 & RF6_SHAPECHANGE) && (!m_ptr->schange))
  ...
  if (m_ptr->hostile == 0)
      ...
      else
	{
	  ...
	  tmp_y = randint(DUNGEON_HGT - 1);
	  tmp_x = randint(DUNGEON_WID - 1);
