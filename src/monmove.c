...
static void find_range(monster_type *m_ptr)
{
  ...
  if (m_ptr->min_range < FLEE_RANGE)
    {
      ...
      if (rf_has(r_ptr->flags, RF_NEVER_MOVE)) m_ptr->min_range += 3;
      
      /* Spellcasters that don't stike never like to get too close */
      if (rf_has(r_ptr->flags, RF_NEVER_BLOW)) m_ptr->min_range += 3;
    }
  ...
  if (rf_has(r_ptr->flags, RF_ARCHER)) m_ptr->best_range += 3;
  ...
  if (r_ptr->freq_ranged > 24)
    {
      ...
      if ((flags_test(r_ptr->spell_flags, RSF_SIZE, RSF_BREATH_MASK, FLAG_END))
...
static void remove_useless_spells(int m_idx, bool los)
{
  ...
  if (m_ptr->mana >= r_ptr->mana) rsf_off(mon_spells, RSF_ADD_MANA);
  
  /* Don't heal if not enough mana to make it useful */
  if (m_ptr->mana < r_ptr->spell_power/4) rsf_off(mon_spells, RSF_HEAL);
  
  /* Don't heal if full */
  if (m_ptr->hp >= m_ptr->maxhp) rsf_off(mon_spells, RSF_HEAL);
  
  /* Don't lash if too far or close */
  if ((m_ptr->cdis > 3) || (m_ptr->cdis < 2)) rsf_off(mon_spells, RSF_LASH);
  
  /* Don't Haste if Hasted */
  if (m_ptr->mspeed > r_ptr->speed +5) rsf_off(mon_spells, RSF_HASTE);
  ...
  if (m_ptr->min_range > 5) rsf_off(mon_spells, RSF_TELE_SELF_TO);
}
...
static int choose_attack_spell_fast(int m_idx, bool random)
{
  ...
  byte spells[32 * RSF_SIZE];
	...
	{
		if (rsf_has(mon_spells, i)) spells[num++] = i;
	}
...
int choose_ranged_attack(int m_idx, bool archery_only, int shape_rate)
{
  ...
  /* No spells left */
  if (rsf_is_empty(mon_spells)) return (0);
   ...
   * restricted list) 
   */
  if ((rf_has(r_ptr->flags, RF_SMART)) && (randint0(10)==0)) rand = TRUE;
  if ((!(rf_has(r_ptr->flags, RF_SMART))) && (randint0(5)==0)) rand = TRUE;
  
  /* Try 'fast' selection first.
   * If there is only one spell, choose that spell.
   * If there are multiple spells, choose one randomly if the 'rand' flag is 
  ...
  for (i = FLAG_START; i < RSF_MAX; i++)
    {
      ...
      if (!rsf_has(mon_spells, i)) continue;
	...
	cur_spell_rating += 2*cur_spell_rating/3;
      ...
      if (rf_has(r_ptr->flags, RF_SMART)) cur_spell_rating *= 16 + randint0(9);
  ...
  if (best_spell_rating < shape_rate)
    {
      ...
      if (p_ptr->wizard) 
	msg_format("Spell rating: %i.", shape_rate);
  ...
  if (shape_rate == -1) return (best_spell_rating);

  /* Otherwise return best spell */
  else return (best_spell);
}
...
bool cave_exist_mon(monster_race *r_ptr, int y, int x, bool occupied_ok)
{
  ...
  if (!(cave_info[y][x] & (CAVE_WALL)))
    {
      ...
      /* Earthbound demons, firebreathers, and red elementals cannot 
       * handle water */
      if (feat == FEAT_WATER)
	{
	  if (rf_has(r_ptr->flags, RF_FLYING)) return (TRUE);
      ...
      if (feat == FEAT_LAVA)
	{
	  if (rf_has(r_ptr->flags, RF_IM_FIRE)) return (TRUE);
      ...
      if (feat == FEAT_VOID)
	{
      ...
      if (feat == FEAT_RUNE_PROTECT) return (FALSE);
  ...
  /* Feature is a wall */
	...
	return (FALSE);
	  ...
	  (rf_has(r_ptr->flags, RF_KILL_WALL))) return (TRUE);
...
static int cave_passable_mon(monster_type *m_ptr, int y, int x, bool *bash)
{
  ...
  if (cave_m_idx[y][x] < 0)
    {
      ...
      if (rf_has(r_ptr->flags, RF_NEVER_BLOW)) return (0);
  ...
  if (!(cave_info[y][x] & (CAVE_WALL)))
      ...
      if (feat == FEAT_WATER)
	{
	  ...
	  if (rf_has(r_ptr->flags, RF_FLYING)) return (move_chance);
	  
	  /* Earthbound demons and firebreathers cannot cross water */
	  if (rsf_has(r_ptr->flags, RSF_BRTH_FIRE)) return (0);
      ...
      if (feat == FEAT_LAVA)
	{
	  ...
	  if (rf_has(r_ptr->flags, RF_IM_FIRE)) return (move_chance);
	  ...
	  (feat == FEAT_SECRET))
	{
	  ...
	  if (rf_has(r_ptr->flags, RF_OPEN_DOOR))
	      ...
	      else if (feat < FEAT_DOOR_HEAD + 0x08)
		{
		  ...
		  if (rf_has(r_ptr->flags, RF_SMART)) ability *= 2;
...
static void get_move_advance(monster_type *m_ptr, int *ty, int *tx)
{
  ...
  if ((rf_has(r_ptr->flags, RF_PASS_WALL)) || (rf_has(r_ptr->flags, RF_KILL_WALL)))
    {
      /* Player is the target */
      if (m_ptr->hostile < 0)
	{
	      ...
	      for (i = 0; i < 8; i++)
		{
		  ...
		  if ((safe_cost[yy][xx] == 0) || 
		      ...
		      if (!player_can_see_bold(yy - conv_y, xx - conv_x))
			{
			  ...
			  /* Accept lower-cost, sometimes accept 
			   * same-cost options */
			  if ((least_cost > this_cost) ||
			      ...
			      for (j = 0; j < 8; j++)
				{
				  ...
				  /* Look for any passable grid that 
				   * isn't in LOS */
				  if ((!player_can_see_bold(yyy, xxx)) &&
				      (cave_passable_mon(m_ptr, yyy, xxx, 
							 &dummy)))
				    {
                     ...
                     bool must_use_target)
{
  ...
  *tx = m_ptr->fx;
  ...
  /* Monster can go through rocks - head straight for target */
  if ((!*fear) && ((rf_has(r_ptr->flags, RF_PASS_WALL)) || 
		   (rf_has(r_ptr->flags, RF_KILL_WALL))))
    {
      *ty = targ_y;
      *tx = targ_x;
	      ...
	      for (j = 0; j < 8; j++)
		{
		  ...
		  if ((( right) && (xx > m_ptr->fx)) ||
		      ((!right) && (xx < m_ptr->fx)))
		    {
		      ...
		      /* Accept (original) grid, but don't 
...
static void make_confused_move(monster_type *m_ptr, int y, int x)
{
  ...
  if (cave_info[y][x] & (CAVE_WALL))
	...
	{
	  if (seen) msg_format("%^s stumbles into the sand.", m_name);
	}
  ...
  /* Feature is not a wall */
	...
	{
	  if (seen) msg_format("%^s stumbles off a cliff.", m_name);
...
static void apply_monster_trap(monster_type *m_ptr, int y, int x, bool *death)
{
  ...
  if (trap_hit)
	...
	{
	  (void)fire_meteor(0, GF_DISENCHANT, y, x, (3 * trap_power) / 4, 
			    0, FALSE);
	  if (!(m_ptr->r_idx)) mon_dies = TRUE;
	}
	...
	{
	  /* 50% - 75% damage of trap power to all creatures within 
	   * LOS of trap*/
	  ...
	  for (i = 1; i < m_max; i++)
	    {
	      ...
	      if (rf_has(r1_ptr->flags, RF_UNIQUE)) continue;
	...
	{
	  if (mon_take_hit(cave_m_idx[y][x], (3 * trap_power) / 4, &fear, 
			   note_dies)) mon_dies = TRUE;
	}
...
static void process_move(monster_type *m_ptr, int ty, int tx, bool bash)
{
  ...
  if ((cave_m_idx[ny][nx] < 0) && (m_ptr->hostile < 0))
    {
      /* SJGU Reset the monster's target (to deal with players 
       * hidden by superstealth) */
      int px = p_ptr->px;
      int py = p_ptr->py;
      m_ptr->tx = px;
      m_ptr->ty = py;
  ...
  if ((cave_info[ny][nx] & (CAVE_WALL)) && (cave_feat[ny][nx] != FEAT_DUNE))
    {
	       ...
	       (feat == FEAT_SECRET))
	{
	  ...
	  if (bash)
	      ...
	      if (cave_info[ny][nx] & (CAVE_SEEN)) 
		{
	      ...
	      else if (OPT(disturb_minor)) disturb(0, 0);
	    }
  ...
  if (do_move)
      ...
      if (rf_has(r_ptr->flags, RF_SMASH_WALL))
	{
	  ...
	  for (i = 0; i < 9; i++)
	    {
	      ...
	      /* Kill it */
	      if ((!(f_ptr->flags & TF_LOS)) || (f_ptr->flags & TF_DOOR_ANY))
		{
		  /* Monster destroyed a wall */
		  did_kill_wall = TRUE;
		  ...
		  /* Note changes to grid - but only if actually seen */
		  if (cave_info[yy][xx] & (CAVE_SEEN)) do_view = TRUE;
		}
      ...
      if ((cave_feat[ny][nx] >= FEAT_MTRAP_HEAD) && 
	  (cave_feat[ny][nx] <= FEAT_MTRAP_TAIL))
	{
	  bool death = FALSE;
      ...
      if (cave_feat[ny][nx] == FEAT_RUNE_MANA)
	{
	  ...
	  if (drain > m_ptr->mana) drain = m_ptr->mana;
	  ...
	  if (m_ptr->ml) msg_format("%^s loses some mana.", m_name);
	}
  ...
  if (m_ptr->ml)
    {
      ...
      if (did_open_door) rf_on(l_ptr->flags, RF_OPEN_DOOR);
      
      /* Monster bashed a door */
      if (did_bash_door) rf_on(l_ptr->flags, RF_BASH_DOOR);
      
      /* Monster tried to pick something up */
      if (did_take_item) rf_on(l_ptr->flags, RF_TAKE_ITEM);
      
      /* Monster tried to crush something */
      if (did_kill_item) rf_on(l_ptr->flags, RF_KILL_ITEM);
      
      /* Monster pushed past another monster */
      if (did_move_body) rf_on(l_ptr->flags, RF_MOVE_BODY);
      
      /* Monster ate another monster */
      if (did_kill_body) rf_on(l_ptr->flags, RF_KILL_BODY);
      
      /* Monster passed through a wall */
      if (did_pass_wall) rf_on(l_ptr->flags, RF_PASS_WALL);
      
      /* Monster destroyed a wall */
      if (did_kill_wall) rf_on(l_ptr->flags, RF_KILL_WALL);
    }
}
...
static void process_monster(monster_type *m_ptr)
{
  ...
  if ((rf_has(r_ptr->flags, RF_TERRITORIAL)) && 
      (stage_map[p_ptr->stage][STAGE_TYPE] != CAVE))
    {
      ...
      int from_home = distance(m_ptr->y_terr, m_ptr->x_terr, m_ptr->fy, 
			       m_ptr->fx);

      /* Step up in units of a fifth of monster detection range */
      for (i = 5; i > 0; i--)
	{
	  if ((from_home * i) > (scan_range * 5)) break;
	}
  ...
  if ((rsf_has(r_ptr->flags, RSF_SHAPECHANGE)) && (!m_ptr->schange))
    shape_rate = assess_shapechange(cave_m_idx[m_ptr->fy][m_ptr->fx], m_ptr);
  ...
  if ((choice == 0) && (rf_has(r_ptr->flags, RF_ARCHER)) && (!(m_ptr->confused)
							  && aware))
    {
  ...
  if (m_ptr->group_leader)
    {
      ...
      if ((!m_ptr->monfear) && 
	  (distance(m_ptr->fy, m_ptr->fx, n_ptr->fy, n_ptr->fx) < scan_range))
	{
	  m_ptr->ty = n_ptr->ty;      
	  m_ptr->tx = n_ptr->tx;
	}
  ...
  if (m_ptr->stasis)
    {
      ...
      if (50 + r_ptr->level + (rf_has(r_ptr->flags, RF_UNIQUE) ? 
      ...
      if ((!m_ptr->stasis) && (m_ptr->ml))
	{
      ...
      else if (m_ptr->energy > p_ptr->energy) m_ptr->energy = p_ptr->energy;
    }
