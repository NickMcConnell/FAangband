  ...
  /*** Handle monster fear -- only for monsters that can move ***/
	...
	*fear = FALSE;
	...
	if (m_ptr->min_range == 1) {
		...
		if (sqinfo_has(cave_info[m_ptr->fy][m_ptr->fx], SQUARE_SEEN)) {
		      ...
		      bool *bash)
{
	...
	for (i = 0; i <= 8; i++) {
		...
		if ((!m_ptr->ml) && (!sqinfo_has(cave_info[oy][ox], SQUARE_SEEN))) {
...
static void apply_monster_trap(monster_type * m_ptr, int y, int x, bool * death)
{
	...
	else if (t_ptr->t_idx == MTRAP_CONF) 
	{
		...
		if (sqinfo_has(cave_info[n_ptr->fy][n_ptr->fx], SQUARE_ICKY))
		    continue;
