		   ...
		   int prob)
{
	...
	if ((in_bounds_fully(y - v_ptr->hgt / 2, x - v_ptr->wid / 2))
		...
		for (xx = x - v_ptr->wid / 2; xx < x + v_ptr->wid / 2; xx++) {
			...
			|| cave_has(cave_info[yy][xx], SQUARE_ICKY))
			good_place = FALSE;
		}
	} else
	    good_place = FALSE;
	...
	if (((cave_feat[ty][tx] != base_feat1)
	    ...
	    || cave_has(cave_info[ty][tx], SQUARE_ICKY)) {
	    free(all_feat);
	    return (total);
	}
	...
	cave_on(cave_info[ty][tx], SQUARE_ICKY);
	...
	for (j = 0; j < 100; j++) {
	    ty += ddy[terrain];
	    tx += ddx[terrain];
	    if (!cave_has(cave_info[ty][tx], SQUARE_ICKY))
		break;
	}
...
void mtn_connect(int y, int x, int y1, int x1)
{
	...
	if ((cave_feat[GRID_Y(gp[j])][GRID_X(gp[j])] == FEAT_ROAD)
	    ...
	    break;
	cave_set_feat(GRID_Y(gp[j]), GRID_X(gp[j]), FEAT_ROAD);
	cave_on(cave_info[GRID_Y(gp[j])][GRID_X(gp[j])], SQUARE_ICKY);
    }
}
...
extern void mtn_gen(void)
{
	...
	for (x = 0; x < DUNGEON_WID; x++) {
	    ...
	    case FEAT_GRASS:
		{
		    ...
		    cave_on(cave_info[y][x], SQUARE_WALL);
		    break;
		}
	    case FEAT_SHOP_HEAD:
		{
...
extern void mtntop_gen(void)
{
	...
	for (x1 = x - a; x1 < x + a; x1++) {
		...
		if (floors == spot) {
		    player_place(y1, x1);
		    cave_on(cave_info[y1][x1], SQUARE_ICKY);
		    continue;
		}
	    }
