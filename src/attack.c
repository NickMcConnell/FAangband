  ...
  if ((power_strike && (rand_int(3) != 0)) || (power > rand_int(500) + 25))
    {
      ...
      if (power_strike) 
	message_format(MSG_HIT,0,"Power Strike! You attempt to confuse %s.", 
		       m_name);
      else message_format(MSG_HIT, 0, "You %s and attempt to confuse %s.", 
			  description, m_name);
    }
  else
    {
      /* Basic attack message. */		
      if (power_strike) message_format(MSG_HIT, 0, "Power Strike! You %s %s.", 
				       description, m_name);
...
bool py_attack(int y, int x, bool can_push)
{
  ...
  if ((!(inventory[INVEN_WIELD].k_idx)) && (bash_chance) &&
      (!(check_ability(SP_UNARMED_COMBAT))) && 
      (!(check_ability(SP_MARTIAL_ARTS))))
    {
      bash_chance *= 4;
    }
	  ...
	  if (check_ability(SP_FURY))
	    {
	      ...
	      boost_value = (3 * p_ptr->energy_use) / 
	  ...
	  if (o_ptr->k_idx)
	    {
		  ...
		  check_ability(SP_MANA_BURN) && (m_ptr->mana))
		{
		  /* Base burn amount */
