...
static void mass_produce(object_type *o_ptr)
{
  ...
  if (st_ptr->type == 0) max_purse = 450;
  if (st_ptr->type == 4) max_purse = 15000;
...
static int store_carry(object_type *o_ptr)
{
  ...
  if (st_ptr->type == STORE_MERCH)
	...
	if (o_ptr->k_idx == st_ptr->table[i]) merch_valid = TRUE;
      if (!merch_valid) return (-1);
    }
...
static bool black_market_crap(object_type *o_ptr)
{
  ...
  while (store_num <= base) base -= MAX_STORES_BIG;
...
static void store_delete(void)
{
  ...
  if (rand_int(100) < 50)
    { 
	  ...
	  (st_ptr->stock[what].tval != TV_SHOT)) num = 1;
    }
  ...
  for (j = 0; j < INVEN_PACK; j++)
    {
      ...
      /* Skip unknown */
      if (!object_aware_p(j_ptr)) continue;
...
static bool order_item_action(char cmd, void *db, int oid)
{
  ...
  if (cmd == '\n' || cmd == '\r' || cmd == '\xff')
      ...
      for (i = 24; i < 32; i++)
	{
      ...
      /* New order */
      else if (avail) st_ptr->table[avail] = idx;
  ...
  for (i = 1; i < z_info->k_max; i++)
    {
      ...
      if ((tval >= TV_MAGIC_BOOK) && (num >= 4)) continue;
...
static void display_order(menu_type *menu, int oid, bool cursor, 
...
void stores_maint(int times)
{
  ...
  for (t = 0; t < NUM_TOWNS; t++)
      ...
      for (n = 0; n < max_stores; n++, m++)
	{
	  for (i = 0; i < times; i++)
	    {
	      ...
	      if (big && (n == (STORE_BLACKM))) continue;
      ...
      for (i = 0; i < times; i++)
	if (big) store_maint(base + STORE_BLACKM);
