...
static int test_difficulty0(void *state) {
...
static int test_values0(void *state) {
	...
	r = parser_parse(p, "values:RES_DARK[40]");
	eq(r, PARSE_ERROR_NONE);
	/* Try setting multiple values at once. */
	r = parser_parse(p, "values:RES_FIRE[40] | RES_COLD[-30]");
	eq(r, PARSE_ERROR_NONE);
	pr = (struct player_race*) parser_priv(p);
	notnull(pr);
	for (i = 0; i < ELEM_MAX; ++i) {
		if (i == ELEM_DARK || i == ELEM_FIRE) {
			eq(pr->el_info[i].res_level, 60);
		} else if (i == ELEM_COLD) {
			eq(pr->el_info[i].res_level, 130);
		} else {
