...
static int test_flags0(void *state) {
	...
	/* Check that specifying an additional flag separately works. */
	r = parser_parse(p, "flags:BENEFICIAL");
	eq(r, PARSE_ERROR_NONE);
	eq(ps->t->flags, TMD_FLAG_NONSTACKING | TMD_FLAG_BENEFICIAL);

	/* Check that specifying multiple flags at once works. */
	ps->t->flags = 0;
	r = parser_parse(p, "flags:BENEFICIAL | NONSTACKING");
	eq(r, PARSE_ERROR_NONE);
	eq(ps->t->flags, TMD_FLAG_NONSTACKING | TMD_FLAG_BENEFICIAL);
