...
static int test_order(void *state) {
	struct parser* p = (struct parser*) state;
	enum parser_error r = parser_parse(p, "combat:4");
...
static int test_name0(void *state) {
	...
	for (i = 0; i < ELEM_MAX; ++i) {
		...
		eq(e->el_info[i].res_level, RES_LEVEL_BASE);
	}
	null(e->brands);
	null(e->slays);
	null(e->curses);
...
static int test_values0(void *state) {
	...
	r = parser_parse(p, "values:INFRA[3] | RES_POIS[30]");
	eq(r, PARSE_ERROR_NONE);
	/* Check the state. */
	for (i = 0; i < OBJ_MOD_MAX; ++i) {
	...
	for (i = 0; i < ELEM_MAX; ++i) {
		eq(e->el_info[i].res_level,
		   ((i == ELEM_POIS) ? RES_LEVEL_BASE - 30 : 0));
	}
	ok;
}
