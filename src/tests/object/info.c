...
int setup_tests(void **state)
{
	...
	/*
	 * Mark the monster as visible since the object information
	 * calculations assume that it is visible.
	 */
	mflag_on(ts->target->mflag, MFLAG_VISIBLE);
		...
		struct object *launcher, bool throw)
{
	...
	while (i < NHITS) {
		if (launcher || throw) {
			...
			assert(weapon);
			if (launcher) {
				ar = make_ranged_shot(p, weapon, m->grid, 0,
					false);
			} else {
				ar = make_ranged_throw(p, weapon, m->grid, 0,
					false);
			}
