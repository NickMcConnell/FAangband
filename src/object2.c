...
void compact_objects(int size)
{
	...
	for (i = 1; i < o_max; i++) {
	     ...
	     * almost worthless) objects that the player is already aware of.
	     * This statement emphasizes speed over smarts. -LM- */
...
void object_absorb(object_type * o_ptr, object_type * j_ptr)
{
    ...
    /* Hack -- if wands are stacking, combine the charges. If one was unknown,
     * now both are. */
    if (o_ptr->tval == TV_WAND) {
	o_ptr->pval += j_ptr->pval;
	if (o_ptr->pval != 0)
	    o_ptr->ident &= ~(IDENT_EMPTY);
	if (!(j_ptr->ident & IDENT_KNOWN))
	    o_ptr->ident &= ~(IDENT_KNOWN);
...
void drop_near(object_type * j_ptr, int chance, int y, int x)
{
	...
	msg_format("The %s disappear%s from view.", o_name,
		   (plural ? "" : "s"));
}
