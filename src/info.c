...
void self_knowledge(bool spoil)
{
    ...
    else if (p_resist_good(P_RES_ACID))
	info[i++] = "You resist acid.";
    else if (p_resist_pos(P_RES_ACID))
	info[i++] = "You resist acid slightly.";
    else if (p_vulnerable(P_RES_ACID))
	info[i++] = "You are vulnerable to acid.";
    if (p_immune(P_RES_ELEC))
	info[i++] = "You are immune to lightning.";
    else if (p_resist_strong(P_RES_ELEC))
	info[i++] = "You resist lightning exceptionally well.";
    else if (p_resist_good(P_RES_ELEC))
	info[i++] = "You resist lightning.";
    ...
    else if (p_resist_good(P_RES_FIRE))
	info[i++] = "You resist fire.";
    else if (p_resist_pos(P_RES_FIRE))
	info[i++] = "You resist fire slightly.";
    else if (p_vulnerable(P_RES_FIRE))
    ...
    else if (p_resist_good(P_RES_COLD))
	info[i++] = "You resist cold.";
    else if (p_resist_pos(P_RES_COLD))
	info[i++] = "You resist cold slightly.";
    else if (p_vulnerable(P_RES_COLD))
	info[i++] = "You are vulnerable to cold.";
    if (p_immune(P_RES_POIS))
	info[i++] = "You are immune to poison.";
    else if (p_resist_strong(P_RES_POIS))
	info[i++] = "You resist poison exceptionally well.";
    else if (p_resist_good(P_RES_POIS))
	info[i++] = "You resist poison.";
    else if (p_resist_pos(P_RES_POIS))
	info[i++] = "You resist poison slightly.";
    else if (p_vulnerable(P_RES_POIS))
	info[i++] = "You are vulnerable to poison.";
    if (p_immune(P_RES_LITE))
	info[i++] = "You are immune to light.";
    else if (p_resist_strong(P_RES_LITE))
	info[i++] = "You resist light exceptionally well.";
    else if (p_resist_good(P_RES_LITE))
	info[i++] = "You resist light.";
    else if (p_resist_pos(P_RES_LITE))
	info[i++] = "You resist light slightly.";
    else if (p_vulnerable(P_RES_LITE))
	info[i++] = "You are vulnerable to light.";
    if (p_immune(P_RES_DARK))
	info[i++] = "You are immune to darkness.";
    else if (p_resist_strong(P_RES_DARK))
	info[i++] = "You resist darkness exceptionally well.";
    else if (p_resist_good(P_RES_DARK))
	info[i++] = "You resist darkness.";
    ...
    if (p_immune(P_RES_SOUND))
	info[i++] = "You are immune to sound.";
    else if (p_resist_strong(P_RES_SOUND))
	info[i++] = "You resist sound exceptionally well.";
    else if (p_resist_good(P_RES_SOUND))
	info[i++] = "You resist sound.";
    else if (p_resist_pos(P_RES_SOUND))
	info[i++] = "You resist sound slightly.";
    else if (p_vulnerable(P_RES_SOUND))
	info[i++] = "You are vulnerable to sound.";
    if (p_immune(P_RES_SHARD))
	info[i++] = "You are immune to blasts of shards.";
    ...
    if (p_immune(P_RES_NEXUS))
	info[i++] = "You are immune to nexus attacks.";
    else if (p_resist_strong(P_RES_NEXUS))
	info[i++] = "You resist nexus attacks exceptionally well.";
    else if (p_resist_good(P_RES_NEXUS))
	info[i++] = "You resist nexus attacks.";
    ...
    else if (p_resist_good(P_RES_NETHR))
	info[i++] = "You resist nether forces.";
    ...
    if (p_immune(P_RES_CHAOS))
	info[i++] = "You are immune to chaos.";
    else if (p_resist_strong(P_RES_CHAOS))
	info[i++] = "You resist chaos exceptionally well.";
    else if (p_resist_good(P_RES_CHAOS))
	info[i++] = "You resist chaos.";
    else if (p_resist_pos(P_RES_CHAOS))
	info[i++] = "You resist chaos slightly.";
    else if (p_vulnerable(P_RES_CHAOS))
	info[i++] = "You are vulnerable to chaos.";
    if (p_immune(P_RES_DISEN))
	info[i++] = "You are immune to disenchantment.";
    ...
    /* Indicate Blessing */
    if (of_has(flags_obj, OF_BLESSED)) {
	info[i++] = "Your weapon has been blessed by the gods.";
    }

    /* Hack */
    if (of_has(flags_obj, OF_IMPACT)) {
...
void spell_info(char *p, int spell_index)
{
    ...
    case 0:
	sprintf(p, " dam %dd3", 4 + plev / 10);
	break;
    case 2:
	strcpy(p, " range 10");
	break;
    case 4:
	sprintf(p, " dam 2d%d, rad %d", 1 + (plev / 5), (plev / 10) + 1);
	break;
    case 5:
	sprintf(p, " dam %d, rad 2", 5 + (plev / 3));
	break;
    case 7:
	sprintf(p, " dur 10+d5");
	break;
    case 9:
	sprintf(p, " dam %dd8, beam %d%%", (2 + ((plev - 5) / 5)), beam);
	break;
    case 13:
	sprintf(p, " range %d", 50 + plev * 2);
	break;
    case 15:
	sprintf(p, " dam %d", 5 + plev);
	break;
    case 20:
	sprintf(p, " range %d", 5 + plev / 10);
	break;
    case 26:
	sprintf(p, " dam %dd%d, length %d", 6 + (plev / 10), 8,
		1 + (plev / 10));
	break;
    case 27:
	sprintf(p, " dam %d, rad 2", 55 + plev);
	break;
    case 30:
	sprintf(p, " dist %d", 45 + (plev / 2));
	break;
    case 34:
	sprintf(p, " dur %d+d%d", plev, plev);
	break;
    case 35:
	strcpy(p, " dur 30+d20");
	break;
	...
	break;
    case 37:
	sprintf(p, " dur %d+d30", 10 + plev);
	break;
    case 38:
	strcpy(p, " dur 40");
	break;
    case 45:
	sprintf(p, " dam %dd8, beam %d%%", (3 + plev / 3), beam);
	break;
    case 49:
	sprintf(p, " dam %d, rad %d", 5 * plev / 2, plev / 12);
	break;
    case 51:
	sprintf(p, " dam %d,%d, rad 3,2", 10 + plev, 2 * plev);
	break;
    case 52:
	sprintf(p, " dam %d, rad 3", 3 * plev);
	break;
    case 53:
	sprintf(p, " dam %d, rad 1", 80 + plev * 2);
	break;
    case 54:
	sprintf(p, " dam %d+3d%d", plev, plev);
	break;
    case 55:
	sprintf(p, " dam %d, rad 1", 80 + plev * 2);
	break;
    case 56:
	sprintf(p, " dam %d, rad %d", 4 * plev, 3 + plev / 15);
	break;
	/* Piety */
    case 65:
	sprintf(p, " heal 2d%d", plev / 4 + 5);
	break;
    case 66:
	strcpy(p, " dur 12+d12");
	break;
    case 68:
	sprintf(p, " dam 2d%d, rad %d", 1 + plev / 3, plev / 10 + 1);
	break;
    case 71:
	strcpy(p, " halve poison");
	break;
    case 72:
	sprintf(p, " heal 4d%d", plev / 4 + 6);
	break;
    case 74:
	sprintf(p, " range %d", 2 * plev);
	break;
    case 75:
	strcpy(p, " dur 24+d24");
	break;
	...
	break;
    case 83:
	sprintf(p, " dur %d+d24", 3 * plev / 2);
	break;
    case 84:
	sprintf(p, " heal 9d%d, any cut", plev / 3 + 12);
	break;
    case 85:
	strcpy(p, " radius 10");
	break;
	...
	break;
    case 94:
	sprintf(p, " dam d%d, heal 300", plev * 4);
	break;
    case 95:
	strcpy(p, " range 10");
	break;
    case 96:
	sprintf(p, " range %d", 4 * plev);
	break;
    case 97:
	sprintf(p, " dist %d", 45 + (plev / 3));
	break;
    case 107:
	strcpy(p, " heal 700, any cut");
	break;
    case 117:
	sprintf(p, " dam %d, rad 3", 2 * plev);
	break;
    case 118:
	sprintf(p, " dam %d", 3 * plev / 2);
	break;
    case 120:
	sprintf(p, " dam %d+d100", plev * 3);
	break;
    case 121:
	sprintf(p, " dam %d, heal 500", plev * 5);
	break;
	/* Nature Magics */
    case 129:
	sprintf(p, " dam 2d%d, rad %d", 1 + plev / 4, plev / 10 + 1);
	break;
    case 131:
	strcpy(p, " range 10");
	break;
	...
	break;
    case 133:
	sprintf(p, " dam %dd6, length %d", 2 + plev / 8, 1 + plev / 5);
	break;
    case 135:
	strcpy(p, " dam 20+d30");
	break;
	...
	break;
    case 138:
	sprintf(p, " dam %dd8, beam %d%%", 2 + plev / 5, beam_low);
	break;
    case 142:
	sprintf(p, " dam %d", 2 + plev / 5);
	break;
    case 143:
	sprintf(p, " dam %dd8, beam %d%%", 3 + plev / 5, beam_low);
	break;
    case 144:
	strcpy(p, " dur 20+d20");
	break;
    case 146:
	sprintf(p, " dam %dd8, beam %d%%", 5 + plev / 5, beam_low);
	break;
    case 147:
	sprintf(p, " dist %d", 45 + (plev / 3));
	break;
    case 148:
	sprintf(p, " dam %dd8, beam %d%%", 5 + plev / 4, beam_low);
	break;
    case 149:
	strcpy(p, " dur 20+d20");
	break;
	...
	break;
    case 153:
	sprintf(p, " heal 2d%d, pois/cut", plev / 5);
	break;
    case 154:
	strcpy(p, " dur 20+d20");
	break;
    case 155:
	sprintf(p, " dam %dd8, conf/slow", plev / 7);
	break;
    case 159:
	sprintf(p, " dam %d+d%d, rad %d", plev, 60 + plev * 2, 1 + plev / 15);
	break;
    case 160:
	sprintf(p, " dam %d+d%d, rad %d", plev, 40 + plev * 2, plev / 8);
	break;
    case 169:
	strcpy(p, " dur 24+d24");
	break;
    case 170:
	sprintf(p, " heal %dd12, any cut", 25 + plev / 2);
	break;
    case 171:
	sprintf(p, " dam %d+d%d, rad %d", plev, 50 + plev * 2, 1 + plev / 12);
	break;
    case 172:
	sprintf(p, " dam %d+d%d, rad %d", 30 + ((4 * plev) / 5), 30 + plev * 2,
		plev / 7);
	break;
    case 173:
	sprintf(p, " dam %d+d%d, rad %d", 3 * plev / 2, 50 + plev * 3,
		1 + plev / 15);
	break;
    case 174:
	sprintf(p, " dam %d+d%d, rad 1", 35 + (2 * plev), 90 + plev * 4);
	break;
    case 175:
	sprintf(p, " dam %d+d%d, rad %d", 40 + (3 * plev / 2), plev * 3,
		plev / 10);
	break;
    case 177:
	sprintf(p, " dur %d+d30", plev / 2);
	break;
    case 178:
	sprintf(p, " dam d%d", plev * 2);
	break;
    case 181:
	sprintf(p, " dam %dd8, beam %d%%", plev / 6, plev * 2);
	break;
    case 182:
	sprintf(p, " dur %d+d10", plev / 2);
	break;
    case 186:
	strcpy(p, " heal 500, dam 100");
	break;
    case 188:
	sprintf(p, " dam 6d6+%d", plev / 3);
	break;
	/* Necromantic Spells */
    case 192:
	sprintf(p, " dam 2d%d", 5 + plev / 7);
	break;
    case 194:
	strcpy(p, " dur 70+d70");
	break;
	...
	break;
    case 201:
	sprintf(p, " dam %dd8, beam %d%%", 3 + plev / 7, beam_low);
	break;
    case 202:
	sprintf(p, " dam %d, poison", 10 + plev);
	break;
    case 206:
	sprintf(p, " dam %d+d%d", plev + 15, 3 * plev / 2);
	break;
    case 207:
	sprintf(p, " dam %d+d%d", plev, plev);
	break;
    case 208:
	sprintf(p, " dur 20+d%d", plev / 2);
	break;
    case 209:
	strcpy(p, " range 16, hurt 1d4");
	break;
    case 215:
	sprintf(p, " dam %dd8, hurt 1d6", 2 + plev / 3);
	break;
    case 216:
	sprintf(p, " dur %d+d20", plev / 2);
	break;
    case 217:
	sprintf(p, " dam %d+d%d", 2 * plev, 2 * plev);
	break;
    case 218:
	sprintf(p, " dam %d", 15 + 7 * plev / 4);
	break;
    case 219:
	sprintf(p, " dam %dd8, beam %d%%", 1 + plev / 2, beam);
	break;
    case 221:
	sprintf(p, " dam %d, rad 2", 50 + plev * 2);
	break;
    case 222:
	sprintf(p, " dam %d+d%d, hit ~9", 50 + plev * 2, plev);
	break;
    case 226:
	sprintf(p, " range %d, hurt 2d6", plev * 3);
	break;
    case 228:
	strcpy(p, " dur 20+d20");
	break;
    case 230:
	sprintf(p, " %d+d%d", plev / 2, plev);
	break;
    case 231:
	strcpy(p, " dur d66");
	break;
	...
	break;
    case 235:
	sprintf(p, " dam %dd13", 3 * plev / 5);
	break;
    case 236:
	sprintf(p, " dam %d, hurt 2d8", 20 + (4 * plev));
	break;
    case 237:
	sprintf(p, " dam %d+4d%d", 60, plev);
	break;
    case 238:
	sprintf(p, " dam %dd11, heal %d", plev / 3, 3 * plev);
	break;
    case 240:
	strcpy(p, " hurt 2d7");
	break;
	...
	break;
    case 247:
	sprintf(p, " dam %d+d50", plev * 3);
	break;
    case 249:
	sprintf(p, " dam %d, rad %d", 11 * plev / 2, plev / 7);
	break;
    case 250:
	sprintf(p, " dur 30+d40");
	break;
    case 251:
	sprintf(p, " dur 40");
	break;
...
void print_spells(int tval, int sval, int y, int x)
{
	...
	/* Analyze the spell */
	if ((i <
	     32) ? ((p_ptr->spell_forgotten1 & (1L << i))) : ((p_ptr->
							       spell_forgotten2
							       & (1L <<
								  (i - 32))))) {
	    comment = " forgotten";
	    ...
	    if (!
		((i <
		  32) ? (p_ptr->spell_learned1 & (1L << i)) : (p_ptr->
							       spell_learned2 &
							       (1L <<
								(i - 32))))) {
	    comment = " unknown";
	    ...
	    if (!
		((i <
		  32) ? (p_ptr->spell_worked1 & (1L << i)) : (p_ptr->
							      spell_worked2 &
							      (1L <<
							       (i - 32))))) {
	    comment = " untried";
	...
	c_put_str(attr_name, format("%-30s", spell_names[s_ptr->index]),
		  y + j + 1, x + 5);
	put_str(format
		("%2d %4d %3d%%", s_ptr->slevel, s_ptr->smana, spell_chance(i)),
		y + j + 1, x + 35);
