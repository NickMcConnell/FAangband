...
static void initialize_artifact(int a_idx)
{
	...
	if ((k_ptr->tval != TV_BOW) && (k_ptr->tval != TV_HAFTED)
	    ...
	    && (k_ptr->tval != TV_HARD_ARMOR) && (k_ptr->tval != TV_DRAG_ARMOR)) {
	    continue;
	}
...
static void choose_basic_theme(int a_idx)
{
    ...
    case TV_HAFTED:
	{
		...
		if ((randint1(6) == 1) && (potential >= 2000)) {
		    ...
		    get_quality(TRUE, SLAY_EVIL, (randint1(3) == 1 ? 7 : 5),
				a_idx);
		    get_quality(TRUE, SLAY_UNDEAD, (randint1(3) == 1 ? 15 : 10),
				a_idx);
		...
		/* Naturally, the appropriate slay. */
		get_quality(TRUE, SLAY_DRAGON, (randint1(3) == 1 ? 15 : 10),
			    a_idx);
		...
		get_quality(TRUE, ADD_AC, randint1(6) + potential / 800, a_idx);
...
static void haggle_till_done(int a_idx)
{
	...
	if ((a_ptr->tval == TV_SWORD) || (a_ptr->tval == TV_POLEARM)
	    ...
	    if ((of_has(a_ptr->flags_obj, OF_THROWING)) && (randint1(2) == 1))
	    ...
	    if (randint1(2) == 1) {
		choice = randint1(8);
		if ((choice == 1) && (!(of_has(a_ptr->flags_obj, OF_SLOW_DIGEST))))
		...
		if ((choice == 5) && (!(of_has(a_ptr->flags_obj, OF_SEE_INVIS))))
		    get_quality(FALSE, SEE_INVIS, 0, a_idx);
		if ((choice == 6) && (!(of_has(a_ptr->flags_obj, OF_FREE_ACT))))
		    get_quality(FALSE, FREE_ACT, 0, a_idx);
		if ((choice == 7) && (!(of_has(a_ptr->flags_obj, OF_FEARLESS))))
