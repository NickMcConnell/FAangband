...
void flavor_init(void)
{
	...
	k_ptr->easy_know = kf_has(k_ptr->flags_kind, KF_EASY_KNOW) ? TRUE : FALSE);
    }
}
 ...
 */
static char *make_inscription(char *buf, size_t buf_sz, int ncomponents,
			      cptr components[])
{

    int i;
    cptr buf_tail = buf + buf_sz - 2;	/* Minus 2 to compensate for
					 * terminating '}' and NUL. */
    bool first = TRUE;
 ...
 */
static void object_desc_full(char *buf, object_type * o_ptr, int pref, int mode,
			     bool unaware_in_store)
{
    char *basenm, *modstr;
    ...
    aware = object_aware_p(o_ptr) || (o_ptr->ident & IDENT_STORE);
    ...
    case TV_AMULET:
	{
	    ...
	    basenm = (OPT(show_flavors)
		      || !(o_ptr->
			   ident & IDENT_WORN) ? "& # Amulet" : "& Amulet");
    ...
    case TV_RING:
	{
	    ...
	    modstr = (k_name + k_ptr->name);
	    basenm = (OPT(show_flavors)
		      || !(o_ptr->ident & IDENT_WORN) ? "& # Ring" : "& Ring");
	...
	&& of_has(o_ptr->flags_obj, OF_THROWING) && (!o_ptr->name1)
	&& of_has(o_ptr->id_obj, OF_PERFECT_BALANCE)) {
	...
	if (of_has(o_ptr->flags_obj, OF_SHOW_MODS) || (show_to_h && show_to_d))
	    show_weapon = TRUE;
	...
	if (is_jewellery(o_ptr)
	    && !((if_has(o_ptr->id_other, IF_TO_H)) || (if_has(o_ptr->id_other, IF_TO_D))));
	...
	/* Hack - delay displaying ring and amulet bonuses */
	if (is_jewellery(o_ptr)
	    && !((if_has(o_ptr->id_other, IF_AC)) || (if_has(o_ptr->id_other, IF_TO_A))));
	 ...
	 * number. */
		...
		if (o_ptr->number > 1) {
		     ...
		     * current timeout by each rod's maximum timeout. Ensure
		     * that any remainder is rounded up.  Display very
		     * discharged stacks as merely fully discharged. */
	...
	/* Hack -- Process Lanterns/Torches */
	    ...
	    object_desc_num_macro(t, o_ptr->pval);
	    object_desc_str_macro(t,
				  (mode == 4 ? "trns)" : " turns of light)"));
	}

	/* Dump bonuses for wearable items */
	...
	if (worn && num_bonus) {
	    ...
	    /* Dump the values */
	    for (j = 0; j < num_bonus; j++) {
		object_desc_int_macro(t, bonus[j]);
		if (j < num_bonus - 1)
		    object_desc_str_macro(t, ", ");
	    }
	    ...
	    if (num_bonus == 1) {
		/* Speed */
		if ((o_ptr->bonus_other[P_BONUS_SPEED] != 0) && (mode != 4)) {
		    /* Dump " to speed" */
		    tail = " to speed";
		}
		...
		/* Magic mastery */
			 ...
			 && (mode != 4)) {
		    /* Dump " to device skill" */
		    tail = " to device skill";
		}
	...
	/* If we're in a store, we want to display if user is aware of the
	 * flavor or not. */
	if (unaware_in_store) {
